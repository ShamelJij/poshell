...
Function jsdir {
    ...
    $ranUserName = (Invoke-RestMethod -Uri "https://randomuser.me/api/?gender=female").results.login.username;
    $ranFirstName = (Invoke-RestMethod -Uri "https://randomuser.me/api/?gender=female").results.name.first;
    $ranCountry = (Invoke-RestMethod -Uri "https://randomuser.me/api/?gender=female").results.location.country;
    $ranEmail = (Invoke-RestMethod -Uri "https://randomuser.me/api/?gender=female").results.email;
    $randomPic = (Invoke-RestMethod -Uri "https://randomuser.me/api/?gender=female").results.picture.thumbnail;
    $page = '<!doctype html>
<html>
  <head>
    <!-- Required meta tags -->
  ...
  <body>
    <nav class="navbar navbar-light bg-light">
      <div><img class="rounded-circle" width="50" height="50" src="' + $randomPic + '"/></div>
      <div>' + $ranEmail + '</div>
      <div>' + $ranUserName + '</div>
      <div>' + $ranFirstName + '</div>
      <div>' + $ranCountry + '</div>
    </nav>
    <h1 class="text-center">' + $ranUserName + '</h1>
    ...
    $randomJSDirectory = $ranUserName;
    $directoryPath = "$env:myStart\randomJS";
    ...
    New-Item -Path $directoryPath -Name  $randomJSDirectory -ItemType Directory;
    New-Item -Path ($directoryPath + "\" + $randomJSDirectory) -Name "index.html" -ItemType File;
